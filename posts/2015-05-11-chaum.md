...
Alice explains the Idea to Yvonee, who looks at the Alice's equation, and tells her that it's all quite simple really, you just need to use _integer addition_[^4] for her Idea to work.
...
So her idea works! Alice wants to try it out right away, so she asks Yvonne to pretend to be Bob, and they begin to work out the details.
...
Look at that! Now the only difference between the two sides is the order of the terms. All we need to assume now is that \\(\\odot\\) is commutative[^12], and we can rearrange the order of the terms however we like:
...
...and that's obviously true. Of course, Yvonne already knows all this, and has another idea for what to use as the \\(\\odot\\) operator.
...
"Associative, commutative, and computationally asymmetric," she says.

Alice knows Yvonne is thinking aloud, so she just waits.

"Modular exponentiation," Yvonne finally says.
